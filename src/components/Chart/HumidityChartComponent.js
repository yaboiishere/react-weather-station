...
const HumidityChartComponent = (props) => {
	...
	let options = {
		...
		scales: {
			yAxes: [{
				...
				ticks: {
					suggestedMin: humidity ? Math.min(...humidity) : -10,
					suggestedMax: humidity ? Math.max(...humidity) : 100,
					padding: 20,
					fontColor: "#9a9a9a",
				},
			}, ],
	...
	return ( 
		<div className = "chart-area" >
			<Line data = {
				chartData
			} //{chartExample1["data1"]} //this.state.weatherStationId]}
			options = {
				options
			}/>
		</div>
...
HumidityChartComponent.propTypes = {};
